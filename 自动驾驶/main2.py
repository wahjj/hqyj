...
def process_detection(image, detector):
    ...
    # 红绿灯处理：红灯停，绿灯行
    if detection_info["traffic_light"] == "red":
        control_params["carSpeed"] = 0  # 红灯停车
    elif detection_info["traffic_light"] == "green":
        control_params["carSpeed"] = 10  # 绿灯行驶

    # 行人处理：有行人减速
    if detection_info["has_person"]:
        control_params["carSpeed"] = 0#max(0, control_params["carSpeed"] - 5)  # 减速

    return control_params, detected_frame


def auto_run(image, mqtt_client, pts_middle, pid, control_params=None):
    ...
    # 只有在提供了control_params且需要变速时才使用检测模块提供的速度
    if control_params is not None:
        # 红灯时停车
        if control_params.get("carSpeed", 10) == 0:
            carspeed = 0
        # 有行人时减速
        elif control_params.get("carSpeed", 10) < 10:
            carspeed = control_params["carSpeed"]
...
if __name__ == '__main__':
    ...
    # pid = PID(Kp=0.3, Ki=0.0305, Kd=0.18, setpoint=240)
    pid.sample_time = 0.1
    pid.output_limits = (-13, 13)

    # 添加检测间隔设置
    DETECTION_INTERVAL = 5  # 每5帧执行一次检测
    detection_frame_counter = 0
    last_control_params = None  # 保存上一次的控制参数
    ...
    while True:
        try:
            ...
            if 'image' in image:
                ...
                # # 1. 交通目标检测（红绿灯、行人等）
                # control_params, detected_image = process_detection(image, detector)
                ...
                # 交通目标检测 - 每隔DETECTION_INTERVAL帧执行一次
                control_params = None
                detection_frame_counter += 1
                if detection_frame_counter >= DETECTION_INTERVAL:
                    control_params, detected_image = process_detection(image, detector)
                    last_control_params = control_params  # 保存当前控制参数
                    detection_frame_counter = 0  # 重置计数器
                else:
                    control_params = last_control_params  # 使用上次的控制参数

                # 自动驾驶
                lane_center, image_center = auto_run(image, mqtt_client, pts_middle, pid, control_params)
            ...
            #怎么检测车道线，为什么要进行透视变换，行人检测原理，怎么实现自动驾驶，为什么要小窗口，红路灯检测原理，红色和黄色车道线的提取和融合原理
            #。